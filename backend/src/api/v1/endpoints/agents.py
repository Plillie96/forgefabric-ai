...
async def run_swarm(
    ...
    try:
        ...
        workflow_id = f"deal-{tenant_id}-{lead.company}-{uuid.uuid4().hex[:8]}"
        ...
        return {"workflow_id": handle.id, "status": "started", "tenant_id": tenant_id}
...
async def approve_decision(
    ...
    try:
        ...
        return {"status": "signaled", "decision": approval.decision, "tenant_id": tenant_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def bill_outcome_endpoint(
    bill: BillInput,
    user=Depends(get_current_user),
    tenant_id: str = Depends(get_current_tenant),
):
